...
from engcalc_colab.native_math import render_responsive_native_results
...
def test_responsive_renderer_emits_native_math_instead_of_raw_dollar_latex():
    ...
    html = render_responsive_native_results([evaluate(engine, "numeric(V(x))")])
...
def test_responsive_native_math_preserves_one_flex_item_per_additive_term():
    ...
    html = render_responsive_native_results([evaluate(engine, "numeric(M(x))")])
