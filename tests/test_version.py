...
def test_runtime_version_is_0_31_2():
    assert engcalc_colab.__version__ == EXPECTED_VERSION


def test_project_metadata_version_is_0_31_2():
    pyproject = tomllib.loads(Path("pyproject.toml").read_text(encoding="utf-8"))
    assert pyproject["project"]["version"] == EXPECTED_VERSION


def test_readme_release_version_is_0_31_2():
    readme = Path("README.md").read_text(encoding="utf-8")
    assert "Current version: **0.31.2**." in readme
    assert readme.rstrip().endswith("Version: `0.31.2`.")
